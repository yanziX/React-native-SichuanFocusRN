...
import ImageAlertView from '../Views/ImageAlertView';
import AlertModal from '../Views/AlertModal';
import Video from 'react-native-video'

const PropTypes = require('prop-types')

export default class SubmissionCell extends Component {
...
videoSetting(videoUrl) {
    ...
    render() {
        ...
        let avatarUrl = api.StaticBaseUrl + newsItem.userAvator;
        ...
        if (newsItem.coverImage === '') {
            let arr = newsItem.discloseImage
            ...
            let videoUrl = api.StaticBaseUrl + newsItem.discloseVideo;
            //视频播放类型
            let videoView = (
            <TouchableOpacity style={{width: 150, height: 200, }} onPress={this.videoSetting.bind(this, videoUrl)}>
                <ImageBackground source={{uri: api.StaticBaseUrl+newsItem.coverImage}}
            ...
            <View style={styles.container}>
                <View style={styles.headerView}>
                    ...
                    <View style={styles.nameAndTimeView}>
                        <Text style={styles.nameText}>{newsItem.userName}</Text>
                        <Text style={styles.pubTimeText}>{newsItem.pubTime}</Text>
                    </View>
                </View>
                <Text style={styles.contentText}>{newsItem.discloseText}</Text>
